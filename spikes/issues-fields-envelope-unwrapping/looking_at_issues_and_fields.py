...
class Client:
    ...
    def refresh_credentials(self):
        ...
        try:
            ...
            error_message = str(e)
            if resp:
                error_message = error_message + ", Response from Jira: {}".format(
                    resp.text
                )
            ...
            self.login_timer = threading.Timer(
                REFRESH_TOKEN_EXPIRATION_PERIOD, self.refresh_credentials
            )
            self.login_timer.start()

    def test_credentials_are_authorized(self):
        self.request("issues", "GET", "/rest/api/2/search", params={"maxResults": 1})
    ...
    def send(self, method, path, headers={}, **kwargs):
        request = requests.Request(
            method, self.url(path), headers=self._headers(headers), **kwargs
        )
...
def spike_on(fields, issues):
    ...
    fields_id_to_name = {
        f.get("id", "NOT_FOUND"): f.get("name", "NOT_FOUND") for f in fields
    }
    processed_issue_nested_fields = {
        fields_id_to_name[k]: v for k, v in issues["fields"].items()
    }
    ...
    write_header(
        "The following are the keys found in `issues`'s top level fields and envelope"
    )
...
def main():
    ...
    try:
        ...
        issue_resp = Context.client.request(
            "issues", "GET", "/rest/api/3/issue/GEN122DCEA-1"
        )

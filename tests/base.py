...
class BaseTapTest(TapSpec, unittest.TestCase):
    ...
    def expected_replication_keys(self):
        ...
        """
        return {
            **self.expected_replication_key_metadata(),
            **{
                table: {key}
                for table, key in self.forced_incremental_streams().items()
                if isinstance(key, str)
            },
            **{
                table: {key["bookmark_path"]}
                for table, key in self.forced_incremental_streams().items()
    ...
    def expected_replication_method(self):
        ...
        return {
            ...
            **{
                key: self.INCREMENTAL
                for key in self.forced_incremental_streams().keys()
            },
        }
    ...
    def top_level_replication_key_fields(self):
        ...
        for stream, fields in self.expected_replication_key_record_paths().items():
            ...
            if len(fields) == 1 and isinstance(peek(fields), tuple):
                fields = {fields.pop()[0]}
    ...
    def setUp(self):
        ...
        if missing_envs:
            raise Exception(
                "Missing test-required environment variables: {}".format(missing_envs)
            )
    ...
    def max_bookmarks_by_stream(self, sync_records):
        ...
        for stream, batch in sync_records.items():
            ...
            stream_record_key = self.expected_replication_key_record_paths().get(
                stream, set()
            )
            stream_bookmark_key = self.expected_replication_keys().get(stream, set())

            self.assertEqual(
                len(stream_bookmark_key),
                1,
                msg="Stream {} has a bookmark key that is not of length 1.\nCompound replication keys and empty replication keys are not supported by max_boomarks.\nBookmark Key: {}".format(
                    stream, stream_bookmark_key
                ),
            )
            ...
            def get_bookmark(message_data, stream_record_key):
                if isinstance(stream_record_key, tuple):
                    ...
                    return get_bookmark(
                        message_data[stream_record_key[0]], stream_record_key[1:]
                    )
                return message_data[stream_record_key]

            bk_values = [
                get_bookmark(message["data"], stream_record_key)
    ...
    def select_all_streams_and_fields(
        ...
        for catalog in catalogs:
            ...
            if (
                catalog["tap_stream_id"] == "issues"
                and "fields" in non_selected_properties
            ):
                non_selected_properties.remove(
                    "fields"
                )  # This contains replication key for issues
            elif (
                catalog["tap_stream_id"] == "worklogs"
                and "updated" in non_selected_properties
            ):
                non_selected_properties.remove(
                    "updated"
                )  # Replication key for worklogs

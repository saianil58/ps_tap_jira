...
class StartDateTest(BaseTapTest):
    ...
    def do_test(self, conn_id):
        ...
        first_total_records = 0
        if first_sync_record_count:
        ...
        bookmark_list = [
            next(iter(book.values())) for stream, book in first_max_bookmarks.items()
        ]
        bookmark_dates = []
        ...
        self.start_date = self.local_to_utc(largest_bookmark).strftime(
                ...
                if target_value:
                    ...
                    try:
                        ...
                        self.assertGreaterEqual(
                            target_value, self.local_to_utc(parse(self.start_date))
                        )

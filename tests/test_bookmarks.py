...
class BookmarkTest(BaseTapTest):
    ...
    def do_test(self, conn_id):
        ...
        our_catalogs = [
            ...
            in incremental_streams.difference(untested_streams)
        ]
        self.select_all_streams_and_fields(
            conn_id, our_catalogs, select_all_fields=False
        )
        ...
        for stream in incremental_streams.difference(untested_streams):
            with self.subTest(stream=stream):
                ...
                print(
                    "Found bookmark key for {}: {}".format(stream, stream_bookmark_key)
                )
                assert (
                    len(stream_bookmark_key) == 1
                ...
                target_value = first_max_bookmarks.get(stream, {None: None}).get(
                    stream_bookmark_key
                )
                ...
                try:
                    ...
                    if state_value:
                        ...
                        if isinstance(state_value, int):
                            state_value = self.local_to_utc(
                                dt.utcfromtimestamp(state_value)
                            )
                ...
                target_value = second_min_bookmarks.get(stream, {None: None}).get(
                    stream_bookmark_key
                )
                try:
                    if target_value:
